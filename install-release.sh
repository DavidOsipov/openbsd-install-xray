...
# This injects environment variables (like XRAY_LOCATION_ASSET) and sets fd limits
# natively, allowing the rc.d script to remain perfectly clean.
setup_login_class() {
...
install_xray() {
    ...
    if [ "$QUIET" -eq 0 ]; then
        ...
        echo "================================================================================"
        echo ""
        echo " [!] IMPORTANT: PORT ISOLATION & PF.CONF LOGIC"
        echo " OpenBSD strictly prohibits unprivileged users (like ${DAEMON_USER}) from binding"
        echo " to privileged ports (< 1024). Do NOT run Xray as root to bypass this."
        echo ""
        echo " Instead, configure Xray to listen on a high port (e.g., 8443) and use pf(4)"
        echo " to redirect traffic. You should also isolate Xray's outbound traffic to"
        echo " prevent routing loops."
        echo ""
        echo " Add the following to your /etc/pf.conf:"
        echo ""
        echo "   # 1. Redirect incoming port 443 to Xray's unprivileged port 8443"
        echo "   pass in on egress inet proto tcp to port 443 rdr-to 127.0.0.1 port 8443"
        echo ""
        echo "   # 2. Allow Xray daemon to make outbound connections (loop prevention)"
        echo "   pass out on egress inet proto tcp from any to any port { 80, 443 } user ${DAEMON_USER}"
        echo ""
        echo " (Remember to run 'doas pfctl -f /etc/pf.conf' after editing)"
        echo "================================================================================"
    else
